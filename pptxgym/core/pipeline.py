...
def drift_of(deck: Deck) -> dict:
    ...
    """
    from ..office import deck_digest
...
def inspect(deck: Deck, dpi: int | None = None, force: bool = False,
    ...
    if roundtrip and (force or not rt_f.exists()):
        from ..office import roundtrip as rtmod
        try:
...
def check_proposal(deck: Deck) -> dict:
    ...
    for t in tasks:
        ...
        if total:
            from ..evaluation.comparators import DECLARATION_SPLIT
...
def check_recipe(deck: Deck) -> dict:
    ...
    from ..office import degrade_exec
...
def degrade(deck: Deck) -> dict:
    ...
    from ..office import degrade_exec, pkg_check
    ...
    def env(self) -> dict:
        ...
        out = {"PPTXGYM_PROBE_MASKED": ":".join(str(p) for p in self.masked),
               ...
               "PYTHONPATH": os.pathsep.join(
                   x for x in (str(Path(__file__).resolve().parents[2]),
...
def check_solvability(deck: Deck) -> dict:
    ...
    from ..orchestration.agent import solvability_rubric_problems
...
#: the plan is not a calibrated plan.  Not a tolerance in the docs/design/reward.md sense —
...
def score_task(deck: Deck) -> dict:
    ...
    """
    from ..evaluation import comparators
...
def harden(deck: Deck, workers: int = 4, wps_workers: int = 2,
    ...
    rejection message.
    """
    from ..evaluation import attacks
...
def consistency_report(deck: Deck) -> dict:
    ...
    """
    from ..evaluation import consistency
